...
namespace AS_Practical_Assignment.Services
{
 ...
 _emailSettings = emailSettings.Value;
    ...
    /// <summary>
 /// Send email using configured SMTP (sends to real email addresses)
        /// </summary>
            ...
            try
  {
   ...
   _logger.LogInformation($"Username: {_emailSettings.Username}");
          _logger.LogInformation($"SSL Enabled: {_emailSettings.EnableSsl}");
      _logger.LogInformation($"Password length: {_emailSettings.Password?.Length ?? 0} characters");
    ...
    _logger.LogInformation($"Sending email from {_emailSettings.SenderEmail} to {toEmail}...");
...
catch (Exception ex)
{
   ...
   /// <summary>
        ...
        public async Task<bool> SendOtpEmailAsync(string toEmail, string otpCode, string userName)
    {
            ...
            {
                _logger.LogWarning("Sending OTP verification email in development environment.");
            }
...
<strong>?? Security Notice:</strong>
 ...
 _logger.LogWarning("?    ?");
   ...
   _logger.LogWarning("?          ?");
                _logger.LogWarning("?  Click to reset: [URL GENERATED – NOT LOGGED]");
                _logger.LogWarning("?          ?");
